...
aime25 = LightevalTaskConfig(
    ...
    few_shots_split=None,
    few_shots_select=None,
    generation_size=4096, #32768, # 4096 for qwen math
    metric=[expr_gold_metric],
    version=1,
)
...
def livemathbench_prompt_fn(line, task_name: str = None):
    """LiveMathBench数据集的prompt生成函数"""
    return Doc(
        task_name=task_name,
...
livemathbench = LightevalTaskConfig(
    name="livemathbench",
    suite=["custom"],
    prompt_function=livemathbench_prompt_fn,
    hf_repo="opencompass/LiveMathBench",
    hf_subset="v202412_AMC_en",  # 根据实际数据集结构调整subset
    hf_avail_splits=["test"],  # 使用测试集进行评估
    evaluation_splits=["test"],
    few_shots_split=None,
    few_shots_select=None,
    generation_size=4096,  # 保持与现有数学任务一致
    metric=[expr_gold_metric],  # 使用表达式匹配指标
    version=1,
)
...
# livemathbench = LightevalTaskConfig(
#     name="livemathbench",
#     dataset_path="../llm_datasets/LiveMathBench",
#     metric=["accuracy"],
...
TASKS_TABLE.append(livemathbench)
