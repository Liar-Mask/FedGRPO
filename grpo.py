...
from open_r1.utils.pre_datasets import get_dataset
...
# 忽略特定的警告消息
warnings.filterwarnings('ignore', message='equations is deprecated')

# 或者忽略来自特定模块的所有警告
warnings.filterwarnings('ignore', module='latex2sympy2_extended.math_normalization')
...
def main(script_args, training_args, model_args):
    ...
    if 'OpenR1-Math' in script_args.dataset_name:
        ...
        # dataset = dataset['train']
        dataset = get_dataset(script_args.dataset_name, None)
        print('dataset #zgx', dataset)
        ...
        # dataset = load_from_disk(script_args.dataset_name)
        dataset = dataset['train']
        expert1_types = ['Algebra', 'Intermediate Algebra']
        expert2_types =['Geometry', 'Number Theory' ]
        expert3_types = ['Counting & Probability', 'Precalculus']
        selected_types= expert1_types
        filtered_samples = dataset.filter(lambda x: x['type'] in selected_types)
        # Print original statistics
        print(f"Original filtered dataset size #zgx: {len(filtered_samples)}")
        shard0, shard1 , shard2= [filtered_samples.shard(num_shards=3, index=i) for i in range(3)]
        dataset = shard1
        # Print shard statistics
        print(f"Shard 0 size: {len(shard0)}")
        print(f"Shard 1 size: {len(shard1)}")
    ################
    # Load tokenizer
    ################
