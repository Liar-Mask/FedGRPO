# GRPO implemention from the HuggingFace Team in open-r1: https://github.com/huggingface/open-r1
...
from exp_dataset.pre_datasets import get_dataset, get_dataset_fedgrpo, get_grpo_reward_dataset, get_dataset_new
...
class GRPOScriptArguments(ScriptArguments):
    ...
    train_mode: str = field(
        ...
        metadata={
            "help": "the mode and object for grpo training",
            "choices": ["reward_train", "normal_train"],
        },
    )
    client_id: int = field(
        default=0,
        metadata={"help": "Under reward training mode, the id of client reward model"},
    )
    problem_types: list[str] = field(
        default_factory=lambda: ["Algebra"],
        metadata={
            "help": "List of math problem types. Possible values: \
...
def main(script_args, training_args, model_args):
    ...
    if script_args.train_mode == 'normal_train':
        dataset = get_dataset(script_args.dataset_name)
    elif script_args.train_mode == 'reward_train':
        dataset= get_grpo_reward_dataset(script_args.dataset_name, script_args.problem_types, script_args.client_id)

    # Note: Use a small dataset for fast check. Should be commented in production!
    # Make sure it's called after data preprocessing

    # script_args.max_num_train_samples = 2000
    ...
    if eval_dataset_name:
        eval_dataset = get_dataset(
            eval_dataset_name, split='test', system_prompt=training_args.system_prompt
        )
